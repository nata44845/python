...
# При решении задачи используйте списочные выражения. Подсказка: проще всего будет найти эллипс в два шага: сначала вычислить самую
...
def find_farthest_orbit(orb):
    sqrs=list(map(lambda a: math.pi*a[0]*a[1] if a[0]!=a[1] else 0,orbits))
    return orbits[sqrs.index(max(sqrs))]
